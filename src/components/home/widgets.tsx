...
import { cn, formatNumber, formatCompact, countdown } from "@/lib/utils";
import { getUser, rankings, battles } from "@/data";
...
export function MiniRanking() {
  const top = rankings.flex.slice(0, 5);
  return (
    <section>
      <RailTitle title="Top Flex da semana" href="/rankings" />
      <Card className="divide-y divide-border">
        {top.map((e) => {
          const u = e.userId ? getUser(e.userId) : undefined;
          if (!u) return null;
          return (
            <Link
              key={e.rank}
...
export function BattleTeaser() {
  const bt = battles[0];
  const [aId, bId] = bt.userIds;
  const a = getUser(aId);
  const b = getUser(bId);
  if (!a || !b) return null;
  const [sa, sb] = bt.scores;
  const total = sa + sb;
  const pct = Math.round((sa / total) * 100);
  return (
    <section>
      <RailTitle title="Batalha de Flex" href={`/flex/battle/${bt.id}`} />
      <Card className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="tnum text-xs text-text-secondary">{formatNumber(sa)}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 text-right">
            <div>
              ...
              <p className="tnum text-xs text-text-secondary">{formatNumber(sb)}</p>
            </div>
...
export function SuggestionPeek({ userId }: { userId: string }) {
