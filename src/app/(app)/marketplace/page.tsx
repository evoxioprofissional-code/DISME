...
import { listDiscordServers } from "@/lib/server-directory";
...
export default async function MarketplacePage({ searchParams }: { searchParams: Promise<{ view?: string; category?: string; q?: string }> }) {
  ...
  const view = params.view === "products" ? "products" : params.view === "showcases" ? "showcases" : params.view === "servers" ? "servers" : "stores";
  ...
  const listings = view === "products" || view === "showcases" ? await listMarketplaceListings({ kind: view === "showcases" ? "showcase" : "market", category, search: params.q }) : [];
  const servers = view === "servers" ? await listDiscordServers(params.q) : [];
  const actionHref = view === "stores" ? "/store/manage" : view === "servers" ? "/servers/manage" : view === "showcases" ? "/marketplace/new?mode=showcase" : "/marketplace/new";
  const actionLabel = view === "stores" ? "Minha loja" : view === "servers" ? "Meus servidores" : view === "showcases" ? "Criar exposição" : "Novo produto";
  return (
    <PageContainer className="max-w-6xl">
      <PageHeader title="Marketplace" subtitle="Lojas, produtos e servidores da comunidade DisMe" action={<Link href={actionHref} className={buttonClasses({ size: "sm" })}>{actionLabel}</Link>} />
      <div className="no-scrollbar mb-5 flex min-w-0 overflow-x-auto border-b border-border" role="tablist" aria-label="Marketplace">
        {([ ["Lojas", "stores"], ["Produtos", "products"], ["Servidores", "servers"], ["Exposições", "showcases"] ] as const).map(([label, key]) => {
          const active = view === key;
          const href = key === "stores" ? "/marketplace" : `/marketplace?view=${key}`;
          return <Link key={key} href={href} role="tab" aria-selected={active} className={cn("relative shrink-0 px-4 py-3 text-sm font-bold transition-colors", active ? "text-text" : "text-muted hover:text-text-secondary")}>{label}{active && <span className="absolute inset-x-3 bottom-0 h-0.5 bg-brand" />}</Link>;
        })}
      </div>
      <form className="relative mb-5" action="/marketplace">
        ...
        <input name="q" defaultValue={params.q} maxLength={60} placeholder={view === "stores" ? "Buscar lojas" : view === "servers" ? "Buscar servidores" : view === "showcases" ? "Buscar exposições" : "Buscar produtos"} className="h-11 w-full rounded-xl border border-border-strong bg-surface pl-10 pr-4 text-sm text-text placeholder:text-muted focus:border-brand focus:outline-none" />
      </form>
      {view === "stores" ? (
        ...
        servers.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{servers.map((server, index) => <ServerCard key={server.id} server={server} priority={index === 0} />)}</div> : <EmptyState title="Nenhum servidor anunciado ainda" description="Donos de servidores podem importar os dados oficiais do Discord e publicar aqui." action={<Link href="/servers/new" className={buttonClasses()}>Anunciar servidor</Link>} />
      ) : view === "showcases" ? (
