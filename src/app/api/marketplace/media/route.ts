...
export async function POST(request: Request) {
  ...
  if (!user) return NextResponse.json({ error: "Entre na sua conta para enviar imagens." }, { status: 401 });
  ...
  if (file.size === 0 || file.size > MAX_FILE_SIZE) {
    return NextResponse.json({ error: "Cada imagem deve ter no máximo 4 MB." }, { status: 400 });
  }
  ...
  return NextResponse.json({ url: admin.storage.from(BUCKET).getPublicUrl(path).data.publicUrl });
}
